...
export const WhatsappFAB = () => {
  const WHATSAPP_NUMBER = environments.VITE_WHATSAPP_NUMBER;
  console.log("WHATSAPP_NUMBER", WHATSAPP_NUMBER);

  if (!WHATSAPP_NUMBER) return null;

  const message = encodeURIComponent(
    "Hola, necesito ayuda con la configuración de mi restaurante.",
  );
  ...
  return (
    <Button
      asChild
      className="fixed bottom-6 right-6 z-50 h-16 w-16 rounded-full shadow-lg hover:shadow-xl transition-shadow p-0"
    >
      <a
        href={href}
        target="_blank"
